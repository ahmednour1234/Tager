<template>
  <!-- Password Reset 5 - Bootstrap Brain Component -->
  <section class="p-3 p-md-4 p-xl-5">
    <div class="container">
      <div class="card border-light-subtle shadow-sm">
        <div class="row g-0">
          <div class="col-12 col-md-6 text-bg-primary">
            <img class="img-fluid rounded-start  w-100 h-100 object-fit-cover" loading="lazy" src="@/assets/logoo.png"
              width="245" height="80" alt="BootstrapBrain Logo">
          </div>
          <div class="col-12 col-md-6">
            <div class="card-body p-3 p-md-4 p-xl-5">
              <div class="row">
                <div class="col-12">
                  <div class="mb-5">
                    <h2 class="h3">Password Reset</h2>
                    <h3 class="fs-6 fw-normal text-secondary m-0">Provide the email address associated with your account
                      to recover your password.</h3>
                  </div>
                </div>
              </div>
              <form @submit.prevent="sendResetLink">
                <div class="row gy-3 gy-md-4 overflow-hidden">
                  <div class="col-12">
                    <label for="email" class="form-label">Email <span class="text-danger">*</span></label>
                    <input type="email" class="form-control" name="email" v-model="email" id="email"
                      placeholder="name@example.com" required>
                  </div>
                  <div class="col-12">
                    <div class="d-grid">
                      <button class="btn bsb-btn-xl btn-primary" type="submit">Reset Password</button>
                    </div>
                  </div>
                </div>
              </form>
              <div class="row">
                <div class="col-12">
                  <hr class="mt-5 mb-4 border-secondary-subtle">
                  <div class="d-flex gap-2 gap-md-4 flex-column flex-md-row justify-content-md-end">
                    <router-link to="/login" class="link-secondary text-decoration-none">Login</router-link>
                    <router-link to="/" class="link-secondary text-decoration-none">Register</router-link>
                  </div>
                </div>
              </div>
              <div class="row">
                <div class="col-12">
                  <p class="mt-5 mb-4">Or sign in with</p>
                  <div class="d-flex gap-3 flex-column flex-xl-row">
                    <a href="#!" class="btn bsb-btn-xl btn-outline-primary">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        class="bi bi-google" viewBox="0 0 16 16">
                        <path
                          d="M15.545 6.558a9.42 9.42 0 0 1 .139 1.626c0 2.434-.87 4.492-2.384 5.885h.002C11.978 15.292 10.158 16 8 16A8 8 0 1 1 8 0a7.689 7.689 0 0 1 5.352 2.082l-2.284 2.284A4.347 4.347 0 0 0 8 3.166c-2.087 0-3.86 1.408-4.492 3.304a4.792 4.792 0 0 0 0 3.063h.003c.635 1.893 2.405 3.301 4.492 3.301 1.078 0 2.004-.276 2.722-.764h-.003a3.702 3.702 0 0 0 1.599-2.431H8v-3.08h7.545z" />
                      </svg>
                      <span class="ms-2 fs-6">Google</span>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
</template>

<script>
import axios from "axios";

export default {
  name: "reset-password",
  data() {
    return {
      email: "",
      showResetForm: false,
      message: "",
      error: "",
      loading: false,
    };
  },
  beforeRouteEnter(to, from, next) {
    // Access the token from the route
    const token = to.params.token;
    next((vm) => {
      // Set the token property in the component
      vm.token = token || "";
    });
  },
  methods: {
    async sendResetLink() {
      try {
        this.loading = true;
        this.error = "";
        this.message = "";

        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(this.email)) {
          this.error = "Please enter a valid email address.";
          this.loading = false;
          return;
        }

        const response = await axios.post("http://127.0.0.1:8000/api/forgot-password", {
          email: this.email,
        });

        this.message = response.data.message;
        this.showResetForm = true;

        if (response.data.message) {
          // Notify the user to check their email
          alert.success("Check your email for further instructions.");
        }
      } catch (error) {
        this.error = error.response ? error.response.data.error : "An unexpected error occurred.";
      } finally {
        this.loading = false;
      }
    },
  },
};
</script>

<style></style>